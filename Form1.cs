...
namespace OtelRezervasyon
{
    public partial class Form1 : Form
    {
        ...
        private async void Form1_Load(object sender, EventArgs e)
        {
            ...
            // Buton rengini ve yazý rengini ayarla (örnek: btngiriþ)
            btngiriþ.BackColor = Color.Red;
            btngiriþ.ForeColor = Color.White; // IndianRed üstünde beyaz yazý okunur
            btngiriþ.FlatStyle = FlatStyle.Flat;
            btngiriþ.FlatAppearance.BorderColor = Color.Red;

            // Label renklerini ve arka planýný ayarla
            foreach (Control ctrl in this.Controls)
            {
                if (ctrl is Label lbl)
                {
                    lbl.ForeColor = Color.DarkRed;      // Okunabilir koyu kýrmýzý
                    lbl.BackColor = Color.DodgerBlue;   // Arka plan formla uyumlu
                }
                else if (ctrl is TextBox txt)
                {
                    txt.BackColor = Color.White;          // TextBox arka plan beyaz
                    txt.ForeColor = Color.DarkRed;      // Yazý rengi IndianRed
                }
            }
